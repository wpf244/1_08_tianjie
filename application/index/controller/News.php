...

{
    ...
    public function invade(){
        ...
        $alig=Request::instance()->param('alig', 0);
      ...
      //  var_dump($post_money);exit;
        ...
        try{
            ...
            foreach($all_queue as $v){
                ...
                $dequeue_id = $v['queue_id']*3; //出队需要的排队人数（当前需要乘以12为出队时的排队序号，减去自己排队需要为出队需要的人数）

              //  $next_monster = ($v['need_kill_number']+1)*$v['queue_id']+$v['queue_id']; //下一个怪物出现的排队序号（当前应当击杀的怪物数量*自己的排队序号为当前出现的最后一只怪物的排队序号）
                $next_monster = $v['queue_id']*3-1;
                $next_monsters = $v['queue_id']*3;
                if($next_monster == $res_two || $next_monsters== $res_two){ //下一个怪物出现的排队序号与新加入排队号相等，给当前排队号增加一个怪物
    ...
    public function kill_monster(){
        ...
        if($queue['need_kill_number'] > $queue['already_kill_number']){
            ...
            if($res){ //合成成功
             //   var_dump($queue['already_kill_number']);
                if($queue['already_kill_number'] + 1 == 3){ //满级
                ...
                if($queue['need_kill_number'] > $queue['already_kill_number']+1){
